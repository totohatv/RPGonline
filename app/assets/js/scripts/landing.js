...
function updateSelectedAccount(authUser){
    let username = '현재 로그인이<br>되어있지 않습니다.<br>로그인을 해주세요!'
    if(authUser != null){
    ...
    sysAEx.on('message', (m) => {
        ...
        if(m.context === 'validateJava'){
            if(m.result == null){
                ...
                setOverlayContent(
                    '호환되는 Java 설치를 찾을 수 없습니다.',
                    'Java 8 64비트 설치가 필요합니다.',
                    '자바 설치',
                    '수동 설치'
                )
                setOverlayHandler(() => {
                    setLaunchDetails('Java 다운로드 준비..')
...
function dlAsync(login = true){
    ...
    aEx.on('error', (err) => {
        loggerLaunchSuite.error('실행 중 오류 발생', err)
        showLaunchFailure('실행 중 오류 발생', err.message || '자세한건 콘솔에서 확인하세요! (CTRL + Shift + i)')
    })
    aEx.on('close', (code, signal) => {
        if(code !== 0){
            loggerLaunchSuite.error(`AssetExec exited with code ${code}, assuming error.`)
            showLaunchFailure('실행 중 오류 발생', '자세한건 콘솔에서 확인하세요! (CTRL + Shift + i)')
        }
    })
            ...
            switch(m.data){
                case 'download':
                    ...
                    if(m.error.code === 'ENOENT'){
                        showLaunchFailure(
                            '다운로드 ERROR',
                            '파일 다운로드 서버에 연결할 수 없습니다. 인터넷에 연결되어 있는지 확인하고 다시 시도하십시오.'
                        )
                    } else {
                        showLaunchFailure(
                            '다운로드 ERROR',
                            '자세한 내용은 콘솔(CTRL + Shift + i)을 참조해보시고 다시 시도해보세요.'
                        )
                    }
            ...
            if(login && allGood) {
                ...
                const gameStateChange = function(data){
                    ...
                    if(SERVER_JOINED_REGEX.test(data)){
                        ...
                        DiscordWrapper.updateDetails('포켓몬 세계로 이동!')
                    }
                }
                ...
                try {
                    ...
                    if(distro.discord != null && serv.discord != null){
                        ...
                        proc.on('close', (code, signal) => {
                            loggerLaunchSuite.log('Discord Rich Presence 종료 중...')
